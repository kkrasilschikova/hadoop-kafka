...
class Processing {
  val http: Regex="^https?://[a-z]+(.*)[a-z]+(.*)[a-z]+(.*)[a-z]+:\\d{1,}/[a-z]+/[a-z0-9]+/[a-z]+/\\d{1,}/\\d{0,}[-_a-zA-Z]+(.*)zip".r
  val archive: Regex="\\d{0,}[-_a-zA-Z]+(.*)zip".r

  def validated(uri: URI,
                pattern: String="^https?://[a-z]+(.*)[a-z]+(.*)[a-z]+(.*)[a-z]+:\\d{1,}/[a-z]+/[a-z0-9]+/[a-z]+/\\d{1,}/\\d{0,}[-_a-zA-Z]+(.*)zip\\?op=OPEN$"): Boolean={
